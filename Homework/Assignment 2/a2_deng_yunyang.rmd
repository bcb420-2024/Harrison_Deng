...
based off the previously used `r alpha` values as thresholds. The upregulation 
and downregulation were also previously determined by the the `r signif_expr` 
variable. The gene lists produced as a product of such thresholds and previous 
gene expression analysis was used as a query set for 
[g:Profiler](https://biit.cs.ut.ee/gprofiler/gost) programmatically and the 
...
system [@dengSuppressionNFkBActivity2018]. In terms of down-regulation, several 
...
There exists various studies supporting what we see in terms of the expression 
level changes of various genes, and their associated participations in pathways.
Some of these, I've already documented near the end of the previous section when
