...
```{r}
phenotype_dge <- DGEList(counts=filtered_controller_vs_art_expr, group=controller_vs_art_metadata$Phenotype)
phenotype_dge <- calcNormFactors(phenotype_dge)
normed_controller_vs_art <- cpm(phenotype_dge)
```
...
plotMDS(phenotype_dge, labels=NULL, pch = 1, col = c("red","blue")[factor(controller_vs_art_metadata$Source)])
legend("topright",
...
plotMDS(phenotype_dge, labels=NULL,pch=1, col = c("red","orange","purple")[factor(controller_vs_art_metadata$Phenotype)])
legend("topright",
